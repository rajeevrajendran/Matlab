...
%        bouton = [db(i).measures.bouton] & [db(i).measures.present] & ([db(i).measures.mito_close]);
        ...
        for j=1:n_shuffles
            ...
            distance2mito_shuf = [distance2mito_shuf ...
                [shuffled_record.measures(bouton).distance2mito] ];
        end
    end
end
...
d=reshape(distance2mito_shuf,n_shuffles,length(distance2mito_shuf)/n_shuffles);
...
logmsg(['Original mean distance2mito = ' num2str(mean(distance2mito_org),3) ' +- ' num2str(sem(distance2mito_org),3) ' um (SEM), n = ' num2str(length(distance2mito_org))]);
logmsg(['Shuffled mean distance2mito = ' num2str(mean(distance2mito_shuf),3) ' +- ' num2str(sem(distance2mito_shuf),3) ' um (SEM), n = ' num2str(length(distance2mito_shuf))]);
...
ylabel('Count');
save_figure(['means_mito_shuffle_distance2mito_' groupname '.png']);
